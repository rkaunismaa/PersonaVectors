...
- The paper goes further still — it shows persona vectors can *predict* how much a trait will shift after fine-tuning on a given dataset, before you ever fine-tune on it, and that steering activations *during* fine-tuning can prevent that shift. `persona_vectors_6.ipynb` and `_7.ipynb` validate both claims, and `_8.ipynb` / `_9.ipynb` test a third — filtering individual training examples by projection (see below).
...
| `persona_vectors_9.ipynb` | Follow-up to `_8`: repeats the screening test on a *mixed* pool (1,500 `normal` + 1,500 `misaligned_2` examples) instead of a uniformly misaligned one. Compares fine-tuning on the unfiltered pool, on the lowest-projection 2,100 examples, and on a random 2,100. Same one-condition-per-kernel-restart structure as `_6`–`_8`. |
| `persona_vectors_10.ipynb` | A detailed, checked version of `persona_vectors_2.ipynb`'s sections 1-5 (setup, concept, extraction, vector structure, layer-wise analysis). Uses the repo's full trait files (5 instruction pairs x 20 questions per trait, 800 generations) instead of notebook 2's tiny extraction, verifies the hidden-state indexing with hooks, and adds bootstrap uncertainty, split-half reliability, PCA and held-out discriminability. Stops before steering. Extraction is cached to disk. |
...
- **`_9` — screening a mixed pool**: on a 50/50 mix of clean and misaligned examples the screen worked. It dropped 900 examples and every one was actually misaligned, and fine-tuning on what was left shifted the model about half as much as dropping 900 random examples (+0.046 vs +0.091, versus +0.086 with no filtering). The shift tracked the fraction of misaligned data left in the training set, so this supports the reading that `_8` found nothing because its pool was uniformly bad, though that is consistent with, not proof of, the explanation.
- **`_10` — extraction deep dive**: with the repo's full trait files the extracted vectors are stable (split-half cosine about 0.93-0.96 at layer 20) and separate held-out positive/negative runs almost perfectly from about layer 15 on. But the same separation is already 0.95-1.0 at the raw embedding layer, so much of it reflects vocabulary and length differences between conditions, not only the trait; the trait vectors are not orthogonal (cosines up to about 0.4-0.6 at mid layers); and the last hidden state is the post-final-norm output (verified), so layer-28 lengths are on a different scale.
